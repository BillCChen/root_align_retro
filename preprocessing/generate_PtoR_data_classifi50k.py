...
def multi_process(data):
    ...
    reaction_class = data['reaction_class'] # 新增行
    pro_mol = Chem.MolFromSmiles(product)
    rea_mol = Chem.MolFromSmiles(reactant)
    """checking data quality"""
    rids = sorted(re.findall(pt, reactant))
    ...
    if return_status['status'] == 0:
        ...
        if data['root_aligned']:
            ...
            for k in range(times):
                ...
                product_tokens = smi_tokenizer(pro_smi)
                # reactant_tokens = smi_tokenizer(reactant_smi)
                ct_token = reaction_class
                assert 0 <= ct_token <= 9, f"Reaction class {ct_token} is out of range (0-9)"
                category_token = str(ct_token) # 假设 reaction_class 是 0-9
                reactant_tokens = f"{category_token} {smi_tokenizer(reactant_smi)}" # 添加类别token
            ...
            category_token = str(reaction_class-1) #新增
            return_status['tgt_data'].append(f"{category_token} {smi_tokenizer(cano_reactanct)}") # 新增
            ...
            for i in range(int(augmentation-1)):
                ...
                category_token = str(reaction_class-1) # 新增
                return_status['tgt_data'].append(f"{category_token} {smi_tokenizer(rea_smi)}") # 新增
...
if __name__ == '__main__':
    ...
    datadir = '/root/reaction_data/USPTO/OOD_official_split_USPTO_50k/'
    savedir = './dataset/USPTO_cls_50k_PtoR_aug{}'.format(args.augmentation)

    savedir += args.postfix
    if not os.path.exists(savedir):
        os.makedirs(savedir)
    for i, data_set in enumerate(datasets):
        csv_path = f"{datadir}/{data_set}.csv"
        csv = pd.read_csv(csv_path)
        reaction_list = list(csv["rxn_smiles"])
        ...
        original_reaction_class_list = list(map(lambda x: int(x)-1, csv['class'])) # 确保原始类别列表不变
        ...
        assert len(multiple_product_indices) == 0 , f"Found {len(multiple_product_indices)} reactions \
            with multiple products, which is not supported. \
            Please preprocess the data to remove these cases."
        ...
        #         original_reaction_class_list.append(original_reaction_class_list[index])

        # for index in multiple_product_indices[::-1]:
        #     del sub_react_list[index]
        #     del sub_prod_list[index]
        #     del original_reaction_class_list[index]
        print(original_reaction_class_list[:50])
        src_data, tgt_data = preprocess(
            ...
            reaction_classes=original_reaction_class_list
        )
