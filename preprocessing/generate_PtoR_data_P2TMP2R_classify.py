...
def preprocess(save_dir, reactants, products,set_name, augmentation=1, reaction_types=None,root_aligned=True,character=False, processes=-1):
    ...
    data = [ {
        ...
        "reaction_type": k
    }  for i,j,k in zip(reactants,products,reaction_types) ]
    src_data = []
    tgt_data = []
    # tmp_data = []
    # tmp_data2 = []
    ...
    for result in tqdm(results):
        ...
        reaction_cls_list.extend([result['reaction_type']] * len(result['src_data']))
    ...
    if augmentation != 999:
        ...
        with open(
            ...
            for tgt in tgt_data:
                f.write('{}\n'.format(tgt))
        with open(
                os.path.join(save_dir, 'reaction_cls-{}.txt'.format(set_name)), 'w') as f:
            for cls in reaction_cls_list:
                f.write('{}\n'.format(cls))
        with open(
...
if __name__ == '__main__':
    ...
    if args.dataset == "USPTO-MIT":
        ...
        if not os.path.exists(savedir):
            os.makedirs(savedir)
        for i, data_set in enumerate(datasets):
            csv_path = f"{datadir}/cls_{data_set}.csv"
            ...
            print("Reaction class distributino: ", reaction_class.value_counts())
